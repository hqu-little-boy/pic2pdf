...
if __name__ == "__main__":
    variate_path = "/home/zengwang/Downloads/新建文件夹/"
    variate_out_path = "./漫画单个输出/"
    # pic2pdf_one.make_file(path, out_path)

    variate_in_path = variate_out_path
    variate_output = "政宗君的复仇.pdf"
    # variate_out_path = "./"

    variate_prefix = '政宗君的复仇 '  # input("请输入PDF文件前缀：")
    variate_suffix = '集'  # input("请输入PDF文件后缀：")
    pic2pdf_one.make_file(variate_path, variate_out_path, variate_prefix, variate_suffix)
    make_file(variate_in_path, variate_output, variate_prefix, variate_suffix, True)
    pic2pdf_one.beep()
